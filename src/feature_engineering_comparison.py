...
def recursive_predict(model, last_values, n_steps, feature_names, future_dates,
                      trend_model=None, train_len=None):
...
# XGBoost - basic
xgb_basic = XGBRegressor(n_estimators=200, max_depth=4, learning_rate=0.1,
                          random_state=42, n_jobs=1, verbosity=0)
xgb_basic.fit(X_basic, y_basic)
basic_pred_xgb = recursive_predict(xgb_basic, train_ts.values, TEST_SIZE,
                                    basic_cols, test_ts.index)
...
basic_pred_rf = recursive_predict(rf_basic, train_ts.values, TEST_SIZE,
                                   basic_cols, test_ts.index)
basic_rf_metrics = compute_metrics(test_ts.values, basic_pred_rf)

logger.info("  XGBoost Basic: RMSE=%.2f, MAE=%.2f, MAPE=%.1f%%", basic_xgb_metrics["RMSE"], basic_xgb_metrics["MAE"], basic_xgb_metrics["MAPE"])
logger.info("  RF Basic:      RMSE=%.2f, MAE=%.2f, MAPE=%.1f%%", basic_rf_metrics["RMSE"], basic_rf_metrics["MAE"], basic_rf_metrics["MAPE"])
...
# XGBoost - full FE
xgb_full = XGBRegressor(n_estimators=200, max_depth=4, learning_rate=0.1,
                          subsample=0.8, colsample_bytree=0.8,
                          reg_alpha=0.1, reg_lambda=1.0,
                          random_state=42, n_jobs=1, verbosity=0)
xgb_full.fit(X_full, y_full)
full_pred_xgb = recursive_predict(xgb_full, detrended, TEST_SIZE,
                                   full_cols, test_ts.index,
                                   trend_model=lr, train_len=len(train_ts))
...
full_pred_rf = recursive_predict(rf_full, detrended, TEST_SIZE,
                                  full_cols, test_ts.index,
                                  trend_model=lr, train_len=len(train_ts))
full_rf_metrics = compute_metrics(test_ts.values, full_pred_rf)

logger.info("  XGBoost Full FE: RMSE=%.2f, MAE=%.2f, MAPE=%.1f%%", full_xgb_metrics["RMSE"], full_xgb_metrics["MAE"], full_xgb_metrics["MAPE"])
logger.info("  RF Full FE:      RMSE=%.2f, MAE=%.2f, MAPE=%.1f%%", full_rf_metrics["RMSE"], full_rf_metrics["MAE"], full_rf_metrics["MAPE"])


# ==============================================================
# Improvement calculations
# ==============================================================
xgb_rmse_imp = (basic_xgb_metrics['RMSE'] - full_xgb_metrics['RMSE']) / basic_xgb_metrics['RMSE'] * 100
...
bars1 = ax.bar(x - width/2, basic_vals, width, label='Sadece Lag (FE Yok)', color='#E74C3C', alpha=0.8)
bars2 = ax.bar(x + width/2, full_vals, width, label='Full FE (Detrend+Rolling+Sin/Cos)', color='#27AE60', alpha=0.8)
...
plt.suptitle('Feature Engineering Etkisi: Ham Lag vs Full FE',
...
ax.plot(test_ts.index, basic_pred_xgb, '#E74C3C', linestyle='--', marker='s', markersize=5,
        linewidth=1.5, label=f'XGBoost Basic (RMSE={basic_xgb_metrics["RMSE"]:.1f})')
ax.plot(test_ts.index, full_pred_xgb, '#27AE60', linestyle='-', marker='D', markersize=5,
        linewidth=2, label=f'XGBoost Full FE (RMSE={full_xgb_metrics["RMSE"]:.1f})')
...
ax = axes[1]
ax.plot(test_ts.index, test_ts.values, 'k-o', markersize=6, linewidth=2, label='Gercek Deger')
ax.plot(test_ts.index, basic_pred_rf, '#E74C3C', linestyle='--', marker='s', markersize=5,
        linewidth=1.5, label=f'RF Basic (RMSE={basic_rf_metrics["RMSE"]:.1f})')
ax.plot(test_ts.index, full_pred_rf, '#27AE60', linestyle='-', marker='D', markersize=5,
        linewidth=2, label=f'RF Full FE (RMSE={full_rf_metrics["RMSE"]:.1f})')
